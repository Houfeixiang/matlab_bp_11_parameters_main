function [pwtt,BF,bp,p,uu,d]=usedbyplot(data,fs)
%Description：
...
choose=1; %choose=1时采用的是改进的7-s法，choose=0时采用的是未改进的7-step滤波法

%[SF,d,uu]=ECG250(ecg,fs);
[~,d,uu]=ECG250(ecg,fs);%%获取R波峰值点的位置信息（存储在d中）
%d峰值点位置，uu归一去线性化ECG。
%BF滤波后的BP

%[max_BP,min_BP,u_bp]=BP250(BP,fs);%%获取BP数据的峰值点位置信息
[BF]=BP250_3(BP,fs);%%获取BP数据滤波后的信号数据，BF为滤波后的BP信号。
...
for i=1:1:l-1
    bg=fs*(50/1000);%定义每相邻两个R波之间使用的PPG数据段的起点（即R波开始后的50ms）
